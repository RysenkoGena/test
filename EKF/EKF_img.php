...
While($obEl = $list->GetNext()){
	...
	foreach ($EKFs as $EKF){
		...
		foreach ($EKF->files as $files){
			...
			//debug($files);
			//echo $files->name."!!<br>";
			if($files -> name == "Основное изображение"){
				//echo $files -> file."!!<br>";
				if(!file_exists($pathToImg.$xml_id.".jpg")){
					if (copy($files -> file, $pathToImg.$xml_id.".jpg")){
						echo "\n\rФайл ".$xml_id.".jpg"." успешно загружен на сервер\n";
					}
				else echo "\n\rКакая-то ошибкa скачивания файла ".$xml_id.".jpg"."\n";
				}
			}
			if($files -> name == "Дополнительное изображение"){
				//echo $files -> file."!!<br>";
				if(!file_exists($pathToImg.$xml_id."_".$j.".jpg")){
					if (copy($files -> file, $pathToImg.$xml_id."_".$j.".jpg")){
						echo "\n\rФайл ".$xml_id."_".$j.".jpg"." успешно загружен на сервер\n";
					}
				else echo "\n\rКакая-то ошибкa скачивания файла ".$xml_id."_".$j.".jpg"."\n";
				}
			}
			$j++;
		}
	...
	echo $text_count."\n";
	//if($i > 10) break; //Полезно на время тестирования
}

function load($artikul, $apiKey){
	echo "загрузка артикула ".$artikul.PHP_EOL;
	$curl = curl_init();
	//echo "https://ekfgroup.com/api/v1/ekf/catalog/files?vendor_code=".$artikul.PHP_EOL;
	curl_setopt_array($curl, array(
		//CURLOPT_URL => "https://ekfgroup.com/api/v1/ekf/catalog/files?vendor_code=rcbo6-1pn-1D-30-ac-av",
		
		CURLOPT_URL => "https://ekfgroup.com/api/v1/ekf/catalog/files?vendor_code=".$artikul,
