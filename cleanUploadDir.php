...
//Папка для бэкапа
$patchBackup = $_SERVER['DOCUMENT_ROOT'] . "/upload/zixnru_Backup/";    //Целевая папка для поиска файлов
$rootDirPath = $_SERVER['DOCUMENT_ROOT'] . "/upload/iblock";    //Создание папки для бэкапа
if (!file_exists($patchBackup)) {
    CheckDirPath($patchBackup);
}
 
// Получаем записи из таблицы b_file
$arFilesCache = array(); $y=0;
$result = $DB->Query('SELECT FILE_NAME, SUBDIR FROM b_file WHERE MODULE_ID = "iblock"');
while ($row = $result->Fetch()) {
    $arFilesCache[$row['FILE_NAME']] = $row['SUBDIR'];
    if (!file_exists($_SERVER['DOCUMENT_ROOT']."/upload/".$row['SUBDIR']."/".$row['FILE_NAME'])){
         //echo "Нет файла для записи".$row['FILE_NAME']."<br>";
         $y++;
         if($y==1) echo $_SERVER['DOCUMENT_ROOT']."/upload/".$row['SUBDIR']."/".$row['FILE_NAME']."<br>";
    }
    //echo $_SERVER['DOCUMENT_ROOT']."/".$row['SUBDIR']."/".$row['FILE_NAME']."<br>";
}
echo "Записей в базе: ".count($arFilesCache)."<br>";
echo "Записей без соответсвующего файла, шт.: ".$y."<br>";

$hRootDir = opendir($rootDirPath);
$count = 0; $contDir = 0; $countFile = 0; $i = 1; $removeFile=0;
...
while (false !== ($subDirName = readdir($hRootDir))) {
    ...
    while (false !== ($fileName = readdir($hSubDir))) {
        ...
        //if ($count>1000){ echo "Уже больше 1000 файлов. Прерываю скрипт. Нужно будет повторить!"; break;}
    }
    closedir($hSubDir);
    //Удалить поддиректорию, если удаление активно и счётчик файлов пустой - т.е каталог пуст
    if ($deleteFiles && !$filesCount)  rmdir($subDirPath);
    $contDir++;
}
if ($count < 1)    echo 'Не нашёл данных для удаления<br>';

if ($saveBackup === 'yes')      echo 'Бэкап файлов поместил в: <strong>' . $patchBackup . '</strong><br>';
